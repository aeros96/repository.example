...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.sportsclassic List-%s]: %s" %(addon_version, string))
    ...
    try:
        ...
        for source in xmldata["channels"]:
            ...
            if ss["genre_title"].lower()==gen or (gen=='sports' and ss["name"][:3] in ['NFL','NHL','NBA','BOX']):
                cname=ss["name"]
                curl=json.dumps(ss)
                cimage=base64.b64decode('aHR0cHM6Ly92aWduZXR0ZS53aWtpYS5ub2Nvb2tpZS5uZXQvbG9nb3BlZGlhL2ltYWdlcy9mL2ZiL05obG5ldHdvcmsucG5nL3JldmlzaW9uL2xhdGVzdD9jYj0yMDE3MDcxMDIxMjczNA==')+ss["logo"]
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         dialog=xbmcgui.Dialog()
         rNo=dialog.select('[COLOR yellow]Pick A Link[/COLOR]', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(Sport Classic,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification([COLORdodgerblue]The Shark[/COLOR] ,[COLORwhite]Enjoy Your Viewing[/COLOR] ,5000)")
        import resolveurl
